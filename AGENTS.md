...
- A citation (WCAG, MDN, Apple HIG, Material Design, GOV.UK Design System, Nielsen Norman Group, Baymard Institute for commerce patterns, or similar) is only added when confident it resolves to a real, correct source. A fabricated citation is worse than no citation — it teaches something false with the appearance of authority. This is the one thing `scripts/validate.py` cannot catch, so it's the one thing a human reviewer should always spot-check before merging new pattern content.
- **No Markdown links in `references/*.md`, ever.** An agent reading these files to check a screen will never click a URL, so a hyperlink is dead weight to the one reader that matters here — links are for human-facing docs (README, ARCHITECTURE.md) only. A citation earns its place one of two ways instead: a bare spec identifier for a formal numbered standard (`WCAG 2.2 SC 3.3.8`, `Apple App Store Review Guideline 3.1.1`), or the finding folded directly into the instruction as a plain, unattributed statement — never a sentence with an organization as its subject ("NN/g recommends," "Baymard found"). See `generation/PROMPT.md`'s "How a citation earns its place" for the full reasoning.
