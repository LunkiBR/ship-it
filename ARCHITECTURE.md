...
### Sourcing update, post-launch

Early feedback on r/UXDesign pointed out a real gap: WCAG, Apple HIG, Material Design, and GOV.UK are solid but none of them is a general UX-research authority, and Nielsen Norman Group (NN/g) — arguably the field's most-cited reference, including Jakob Nielsen's original 10 usability heuristics — wasn't in the source list at all. A second, narrower gap: Baymard Institute publishes the deepest evidence-based research specifically on cart/checkout/pricing (checkout field counts, guest-checkout prominence, cost-transparency findings), more specific than generic commerce advice for exactly the patterns this project already has.

Both are now in the accepted source list (`generation/PROMPT.md`, `AGENTS.md`'s Development Rules). `login.md` got the first two NN/g citations — see the two flagged items above. A follow-up research pass (a background agent, fetching and quoting every candidate page directly rather than trusting search-engine summaries, which it caught fabricating a citation mid-task) then went through the rest of the catalog and found a specific, fetch-verified NN/g or Baymard citation for 16 more patterns: `checkout.md`, `cart.md`, `pricing.md`, `onboarding.md`, `empty-state.md`, `404.md`, `search.md`, `settings.md`, `admin-panel.md`, `faq.md`, `testimonials.md`, `sign-up.md`, `paywall.md`, `resetting-password.md`, `showing-input-error.md`, `contact-us.md`. `account.md` came up empty: nothing found was specific to its actual sub-behaviors (OAuth linking, deactivate-vs-delete, re-auth before a security change), and the closest matches were generic error-prevention principles, so no citation was forced in.

The first pass at applying this research just appended a "why it matters" clause to an existing bullet's own wording — which, on review, is weaker than it should be: a citation should sharpen *what to build*, not just justify what's already written. Reworked with that bar: most bullets now lead with a concrete instruction the source actually gave (e.g. `checkout.md`'s keyboard item now names the specific `autocorrect`/`autocapitalize`/`novalidate` attributes Baymard's audit checked for, not just their failure rate), and three findings that didn't fit any existing bullet became new checklist items instead of being forced into an unrelated one: `checkout.md` gained **Guest checkout as the prominent option** (Baymard: 62% of sites bury it, 18% of users abandon over forced account creation), `settings.md` gained **Immediate-effect toggle behavior** as its own item separate from *which widget* to use (NN/g: a toggle applies its state instantly, no Save button), and `contact-us.md` gained **No account required to make contact** (NN/g states this as its own rule, distinct from the existing form-brevity and routing items).

A second round of feedback caught a deeper problem with all of this: the citations above, as first written into `references/*.md`, were Markdown links — `[NN/g recommends](url)`, `[WCAG 2.2 SC 3.3.8](url)`, and so on. That's backwards for who actually reads these files. `SKILL.md`'s whole workflow only ever has an agent open a `references/*.md` file to check it against a screen — nothing in that workflow ever has the agent follow a link, so a hyperlink there is dead weight: tokens spent on a URL nobody will fetch, in a file whose entire job is being agent-facing working material. Links, and the write-up of *why* something is cited, belong in human-facing docs like this one and the README — not in the file the agent actually loads. Every citation across all 23 affected `references/*.md` files (not just the new NN/g/Baymard ones — this included pre-existing WCAG/Apple/Material links in `login.md`, `billing.md`, `paywall.md`, `search.md`, `showing-input-error.md`, `submitting-a-form.md`, `adding-to-cart.md`, `single-item-detail.md`, `tab-bar-navigation.md`, and `account.md`, present since the original authoring pass) was rewritten to drop the link and the "X recommends/found" framing, keeping only a bare spec identifier for formal numbered standards (`WCAG 2.2 SC 3.3.8`, `Apple App Store Review Guideline 3.1.1`) or the finding folded directly into the instruction as a plain fact, with unsourced persuasive statistics (an abandonment percentage, a failure rate) dropped entirely rather than left floating with no attribution and no way to verify them. The full rule is now documented in `generation/PROMPT.md`'s "How a citation earns its place" and `AGENTS.md`'s Development Rules, so future contributions don't reintroduce it.

One more thing worth recording precisely, since it's the exact kind of claim this project is careful about: the commenter's "2,397 UX design guidelines" figure is real — it's live anchor text on NN/g's own Ten Usability Heuristics article, linking to their reports catalog — but it's NN/g's self-reported, evolving aggregate across their whole paid+free report library, not a fixed research count or a breakdown either endpoint explains. Cite it as that, not as a dated statistic.
...
- A confident NN/g citation for `account.md` — searched for and not found (see "Sourcing update" above). Revisit if a good source ever surfaces rather than forcing a weak one in now.
