...
- [ ] **Credential autofill** — Supports the platform's password manager: on web, `autocomplete="username"` on the identifier field and `autocomplete="current-password"` on the password field (`new-password` is for signup/change, not sign-in); on iOS, the matching `textContentType`; on Android, participation in Autofill Framework/Credential Manager. Never set `autocomplete="off"` on these fields — most browsers ignore it for password managers anyway, but it still blocks some assistive tools.
- [ ] **Password field accepts paste** — Never block pasting into the password field; it's how password managers and browser-generated passwords get in — one of the most common, avoidable password-field mistakes.
...
- [ ] **Non-revealing error states** — Distinguishes "check your credentials" without confirming which field was wrong, for the same anti-enumeration reason as password reset — precise and human-readable, never a bare "Error" or "Invalid input."
- [ ] **Rate limiting / lockout feedback** — After repeated failed attempts, show a cooldown message rather than a silent, unexplained block. A CAPTCHA is a common choice here but is itself a cognitive-function test under WCAG 2.2 SC 3.3.8 ("solve a puzzle") — pair it with a non-puzzle alternative (e.g. an email link) if this needs to stay accessible.
...
Notice what these two files actually do: roughly 6–16 items each, every item is one to three sentences, at least 2–3 items per file are grounded in a specific real standard rather than a vague "follow best practices," platform-specific nuance is marked inline with `(Mobile)` or folded into the sentence rather than split into a separate section, and the closing `## Notes` is used sparingly — only for one thing that doesn't fit as a checkbox, omitted entirely if there's nothing that qualifies. Notice too what they *don't* do: no Markdown links and no sentence structured as "X recommends" or "X found" — see "How a citation earns its place" below for why and how to write one instead.
...
- **No Markdown links, ever, anywhere in the file.** These files are read by an agent that will never click a URL — a hyperlink is pure dead weight to that reader. Links belong only in human-facing docs (README, ARCHITECTURE.md), which you are not writing.

# How a citation earns its place (read this before citing anything)

A citation only helps if it sharpens the instruction itself, not if it just proves someone said something. Never write a sentence with an organization as its grammatical subject — no "NN/g recommends," "Baymard found," "WCAG requires," "Apple's guideline states." A reader (human or agent) can't act on being told who said a thing; they can only act on what to actually do. Two ways a real citation shows up here instead:

1. **A bare, compact spec identifier**, for a formal, numbered standard only — `WCAG 2.2 SC 3.3.8`, `Apple App Store Review Guideline 3.1.1`, `Material Design accessibility guidelines`. These read as regulatory tags, not opinions, so a plain parenthetical is enough: `...(WCAG 2.1 SC 3.3.1, Error Identification)`. No "per," no link, no verb.
2. **Fold the finding directly into the instruction as a plain, unattributed statement of fact.** This is the only option for NN/g, Baymard, or any research/opinion source — they don't have numbered spec sections, so naming them adds nothing an agent can use. If the source gives a number that *is itself the instruction* (a field-count limit, a size threshold), keep the number and drop the source name: "limited to 3-5 fields," not "NN/g recommends limiting to 3-5 fields." If the source gives a persuasive statistic that's only *evidence the practice matters* (a failure rate, an abandonment percentage), drop the statistic too — state the practice as a plain instruction and nothing else. A specific unsourced percentage reads as a possibly-invented stat, which is worse than no number at all; a plain instruction ("show the shipping estimate in the cart, not at final checkout") needs no number to be actionable.

When you're not sure whether a piece of research clears this bar, ask: does the agent need to know *who* said this, or just *what to do*? It is always the second one.
...
- **Nielsen Norman Group** (`nngroup.com/articles/`) — for usability heuristics (Jakob Nielsen's original 10, e.g. Visibility of System Status, Error Prevention, Recognition Rather than Recall) and their research articles on specific mechanics (error message wording, empty states, form design). Cite a specific NN/g article, not just "NN/g heuristics" as a vague gesture — the same specificity bar as every other source here.
- **Baymard Institute** (`baymard.com/research/`) — for e-commerce-specific patterns (Cart, Checkout, Pricing, Making a card payment): their published research (checkout field counts, guest-checkout prominence, cost-transparency findings) is more specific and evidence-based than generic commerce advice, and applies narrowly to those patterns rather than across the board.

Not every item needs a citation — the two examples above cite maybe a quarter of their items. Forcing a citation onto every line produces the same generic-filler problem as having none. These sources are where you research and verify a fact — the URL itself never appears in the output file; see "How a citation earns its place" above for the only two ways a finding from any of these sources should actually show up in the Markdown you write.
